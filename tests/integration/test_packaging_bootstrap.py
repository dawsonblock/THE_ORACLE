...
def test_bootstrap_installs_editables_without_pythonpath_hacks() -> None:
    ...
    assert "PYTHON_BIN=" in bootstrap
    assert "command -v" in bootstrap
    assert "sys.version_info < (3, 11)" in bootstrap
    # Venv creation uses the detected Python
    assert '"$PYTHON_BIN" -m venv' in bootstrap
    ...
    # Editable installs can be in requirements.txt or explicit in bootstrap
    assert ("pip install -e third_party/aider" in bootstrap or "-e third_party/aider" in requirements)
    assert "pip install -e ." not in bootstrap
    assert ("pip install -e third_party/code-agent-runtime" in bootstrap or "-e third_party/code-agent-runtime" in requirements)
    assert ("pip install -e third_party/cocoindex-code" in bootstrap or "-e third_party/cocoindex-code" in requirements)
    # Import check uses venv Python
